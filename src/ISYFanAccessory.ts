...
export class ISYFanAccessory extends ISYDeviceAccessory<InsteonFanDevice, Categories.FAN> {
	...
	public map(propertyName): {characteristic: typeof Characteristic, service: Service} {
		super.map(propertyName);
		if (propertyName === 'motor.ST') {
			return  {characteristic: Characteristic.RotationSpeed,service: this.fanService};
		}
		else if (propertyName === 'light.ST') {
			return { characteristic: Characteristic.Brightness, service: this.lightService };
		}
