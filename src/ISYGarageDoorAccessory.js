...
class ISYGarageDoorAccessory extends ISYAccessory_1.ISYAccessory {
    constructor(log, sensorDevice, relayDevice, name, timeToOpen, alternate) {
        ...
        if (this.getSensorState()) {
            ...
            this.targetGarageState = homebridge_1.Characteristic.TargetDoorState.OPEN;
            this.currentGarageState = homebridge_1.Characteristic.CurrentDoorState.OPEN;
        }
        else {
            this.logger(`GARAGE: ${this.name} Initial set during startup the sensor is closed so defaulting states to closed`);
            this.targetGarageState = homebridge_1.Characteristic.TargetDoorState.CLOSED;
            this.currentGarageState = homebridge_1.Characteristic.CurrentDoorState.CLOSED;
        }
    }
    getSensorState() {
        if (this.alternate) {
    ...
    setTargetDoorState(targetState, callback) {
        ...
        if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.OPEN) {
            if (targetState === homebridge_1.Characteristic.TargetDoorState.CLOSED) {
                this.logger(`GARAGE: Current state is open and target is closed. Changing state to closing and sending command`);
                this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.CurrentDoorState, homebridge_1.Characteristic.CurrentDoorState.CLOSING);
                this.sendGarageDoorCommand(callback);
            }
        }
        else if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.CLOSED) {
            if (targetState === homebridge_1.Characteristic.TargetDoorState.OPEN) {
                this.logger(`GARAGE: Current state is closed and target is open. Waiting for sensor change to trigger opening state`);
                this.sendGarageDoorCommand(callback);
                return;
            }
        }
        else if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.OPENING) {
            if (targetState === homebridge_1.Characteristic.TargetDoorState.CLOSED) {
        ...
        else if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.CLOSING) {
            if (targetState === homebridge_1.Characteristic.TargetDoorState.OPEN) {
                ...
                this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.CurrentDoorState, homebridge_1.Characteristic.CurrentDoorState.OPENING);
                this.sendGarageDoorCommand(function () {
                    that.sendGarageDoorCommand(callback);
    ...
    completeOpen() {
        if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.OPENING) {
            this.logger('Current door has bee opening long enough, marking open');
            this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.CurrentDoorState, homebridge_1.Characteristic.CurrentDoorState.OPEN);
        }
        else {
    ...
    handleExternalChange(propertyName, value, formattedValue) {
        ...
        if (this.getSensorState()) {
            if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.OPEN) {
                ...
                this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.CurrentDoorState, homebridge_1.Characteristic.CurrentDoorState.OPENING);
                this.targetGarageState = homebridge_1.Characteristic.TargetDoorState.OPEN;
                this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.TargetDoorState, homebridge_1.Characteristic.CurrentDoorState.OPEN);
            ...
            if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.OPEN) {
                ...
                this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.CurrentDoorState, homebridge_1.Characteristic.CurrentDoorState.CLOSED);
                this.targetGarageState = homebridge_1.Characteristic.TargetDoorState.CLOSED;
                this.garageDoorService.setCharacteristic(homebridge_1.Characteristic.TargetDoorState, homebridge_1.Characteristic.TargetDoorState.CLOSED);
            }
            else if (this.currentGarageState === homebridge_1.Characteristic.CurrentDoorState.CLOSED) {
    ...
    getServices() {
        ...
        garageDoorService.getCharacteristic(homebridge_1.Characteristic.TargetDoorState).on('set', this.setTargetDoorState.bind(this));
        garageDoorService.getCharacteristic(homebridge_1.Characteristic.TargetDoorState).on('get', this.getTargetDoorState.bind(this));
        garageDoorService.getCharacteristic(homebridge_1.Characteristic.CurrentDoorState).on('get', this.getCurrentDoorState.bind(this));
        garageDoorService.getCharacteristic(homebridge_1.Characteristic.CurrentDoorState).on('set', this.setCurrentDoorState.bind(this));
        garageDoorService.getCharacteristic(homebridge_1.Characteristic.ObstructionDetected).on('get', this.getObstructionState.bind(this));
