...
exports.toFahrenheit = toFahrenheit; // export function onGetAsync<T>(character: characteristic.Characteristic, func: (arg: CharacteristicValue) => Promise<T>): characteristic.Characteristic {
...
logger_1.Logger.prototype.trace = (...msg) => {
  const log = this; // onst newMsg = chalk.dim(msg);
...
logger_1.Logger.prototype.fatal = (...msg) => {
  ...
  const log = this; // const newMsg = chalk.dim(msg);
...
function addCallback(func) {
  return (arg, cb) => {
    ...
    console.log(arg); // const n = newArgs[1];
