...
class ISYMotionSensorAccessory extends ISYDeviceAccessory_1.ISYDeviceAccessory {
    ...
    constructor(device) {
        super(device);
        this.category = 10 /* SENSOR */;
    }
    map(propertyName, propertyValue) {
        // let o = super(propertyValue,propertyValue);
        switch (propertyName) {
            case 'CLITEMP':
                return { characteristicValue: utils_1.toCelsius(propertyValue), characteristic: hap_nodejs_1.Characteristic.CurrentTemperature, service: this.temperatureSensorService };
            case 'BATLVL':
                return { characteristicValue: propertyValue, characteristic: hap_nodejs_1.Characteristic.BatteryLevel, service: this.batteryLevelService };
            case 'ST':
                return { characteristicValue: propertyValue, characteristic: hap_nodejs_1.Characteristic.Active, service: this.informationService };
            case 'LUMIN':
                return { characteristicValue: propertyValue, characteristic: hap_nodejs_1.Characteristic.CurrentTemperature, service: this.lightSensorService };
            case 'motionDetected':
                return { characteristicValue: propertyValue, characteristic: hap_nodejs_1.Characteristic.MotionDetected, service: this.motionSensorService };
        }
        return null;
    }
