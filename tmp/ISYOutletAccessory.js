...
class ISYOnOffOutletAccessory extends ISYDeviceAccessory_1.ISYDeviceAccessory {
    ...
    get outlet2Service() {
        if (!this._outlet2Service)
            this._outlet2Service = this.platformAccessory.addService(new hap_nodejs_1.Service.Outlet('Outlet 2', '2'));
        return this._outlet2Service;
    }
    map(propertyName, propertyValue) {
        if (propertyName === 'outlet1.ST') {
            return { characteristicValue: propertyValue, characteristic: hap_nodejs_1.Characteristic.On, service: this.outlet1Service };
        }
        else if (propertyName === 'outlet2.ST') {
            return { characteristicValue: propertyValue, characteristic: hap_nodejs_1.Characteristic.On, service: this.outlet2Service };
        }
    }
    setupServices() {
        super.setupServices();
...
class ISYOutletAccessory extends ISYDeviceAccessory_1.ISYDeviceAccessory {
    ...
    setupServices() {
        ...
        outletService.getCharacteristic(hap_nodejs_1.Characteristic.OutletInUse).onGet(() => true); /*Insteon Outlets Do not report this*/
    }
}
